...
public class NoteDaoTest {
    ...
    public void testCategoryFiltering() throws Exception {
        Note workNote = new Note("Work Task", "Content", System.currentTimeMillis(), "Work", -1, false, false);
        Note personalNote = new Note("Personal Task", "Content", System.currentTimeMillis(), "Personal", -1, false, false);
        
        noteDao.insert(workNote);
        noteDao.insert(personalNote);
        
        List<Note> workNotes = LiveDataTestUtil.getOrAwaitValue(noteDao.getNotesByCategory("Work"));
        assertEquals(1, workNotes.size());
        assertEquals("Work Task", workNotes.get(0).getTitle());
    }
}
